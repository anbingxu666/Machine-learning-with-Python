...
class my_knn_classifier:
    ...
    def _predict(self,x):
        ...
        res_index=np.argsort(distance)
        first_k_item=[self._Y_train[the_k_item] for the_k_item in res_index[:self.k]]
        predict_result=Counter(first_k_item).most_common(1)[0][0]
